...
<div class="forum-topic">
  ...
  <?php if ($fields['taxonomy_forums']): ?>
    <div class="views-field views-field-taxonomy-forums">
      Objavljen na <a href="/forum/<?php print str_replace(' ', '-', strtolower($row->field_taxonomy_forums[0]['raw']['taxonomy_term']->name)); ?>"><?php print $row->field_taxonomy_forums[0]['raw']['taxonomy_term']->name; ?></a>
    </div>
  <?php endif; ?>
  <?php if($new): ?>
...
<div class="forum-topic-replies">
  <?php if ($fields['created_1']->raw): ?>
    <div class="reply-counter">
      <a href="<?php print drupal_get_path_alias('node/' . $row->nid) . '#comments'; ?>">
        ...
        <?php if ($new_replies): ?>
          <span class="new"><?php print $new_replies; ?> novi</span>
        <?php endif; ?>
      </a>
    </div>
  ...
  <?php else: ?>
    <p class="no-replies grey-text">Nema odgovora</p>
  <?php endif; ?>
</div>
