...
CKAN.GA_Reports.render_rickshaw = function( css_name, data, mode, colorscheme ) {
    ...
    if (!Modernizr.svg) {
        renderError('','','(Nema dostupnog grafa)');
        return;
    }
    if (data.length==0) {
        renderError('alert-info','Nema dostupnog grafa','(Nema dostupnog grafa)');
        return
    }
    ...
    myLegend.prepend('<div class="instructions">Kliknite na seriju kako bi izolirali graf:</div>');
    graph.render();
};
