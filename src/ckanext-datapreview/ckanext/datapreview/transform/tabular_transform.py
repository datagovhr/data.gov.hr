...
class TabularTransformer(base.Transformer):
    ...
    def transform(self):
        ...
        if len(tables) > 1:
            extra = "Samo 1 od {0} tablica prikazana".format(len(tables))
            if row_count > self.max_results:
                extra = extra + " i {0} redaka {1} u tablici".format(self.max_results, row_count)
        else:
            if row_count > self.max_results:
                extra = "Prikazano je {0} redaka od {1}".format(self.max_results, row_count)
        ...
        result = {
            ...
            "extra_text": extra,
            "archived": "Datoteka je prikazana iz data.gov.hr arhive." if self.from_archive else ""
        }
