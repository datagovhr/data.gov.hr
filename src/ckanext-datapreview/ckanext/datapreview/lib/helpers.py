...
def get_resource_length(url, resource, required=False, redirects=0):
    ...
    if required:
        ...
        raise ResourceError("Informacije",
            'Nema podataka o duljini skupa podataka: %s' % \
                                identify_resource(resource))
    return None
...
def proxy_query(resource, url, query):
    ...
    try:
        ...
        raise RequestError('Resource type not supported',
            'Transformacija resursa tipa %s nije izvodljiv.'
            % (resource_type))
    ...
    if query.get('archived', True):
        ...
        if length and trans.requires_size_limit and int(length) > max_length:
            raise ResourceError('Datoteka je prevelika za pretpregled',
                                'Datoteka je velika %s. '
                                'Limit je %s. Resurs: %s'
                                % (sizeof_fmt(length),
    ...
    try:
        ...
        log.debug('Transformacija za %s nije uspjela. %s', url, si)
        raise ResourceError("Data Transformation Error",
            "Dogodio se problem prilikom otvaranja podataka: %s" % \
                                    identify_resource(resource))
    except Exception, e:
        log.exception(e)
        raise ResourceError("Data Transformation Error",
                            "Transformacija nije uspjela: %s %s" % \
                                    (identify_resource(resource), e))
