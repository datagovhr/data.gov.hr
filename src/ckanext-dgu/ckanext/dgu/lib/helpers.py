...
def get_cache(resource_dict):
    ...
    # on a machine other than data.gov.hr
    url = url.replace('http://data.gov.hr/', '/')
...
def render_qa_info_for_resource(resource_dict):
    ...
    if not qa:
        return 'To be determined'
    reason_list = (qa['openness_score_reason'] or '').replace('Razlog: Download error. ', '').replace('Error details: ', '').split('. ')
...
def render_stars(stars, reason, last_updated):
    ...
    tooltip = t.literal('<div class="star-rating-reason"><b>Razlog: </b>%s</div>' % reason) if reason else ''
    ...
    if last_updated:
        ...
        tooltip += t.literal(u'<div class="star-rating-last-updated"><b>Bodovi ažurirani: </b>%s</div>' % datestamp)
...
def get_resource_fields(resource, pkg_extras):
    ...
    field_value_map = {
        ...
        'date-updated-computed': {'label': 'Promijenjeno', 'label_title': 'Promijenjeno na data.gov.hr', 'value': render_datestamp(res_dict.get('revision_timestamp'))},
        'content_type': {'label': 'Content Type', 'value': ''},
        'scraper_url': {'label': 'Scraper',
            'label_title':'URL of the scraper used to obtain the data',
            'value': t.literal(scraper_icon(c.resource)) + h.link_to(res_dict.get('scraper_url'), 'https://scraperwiki.com/scrapers/%s' %res_dict.get('scraper_url')) if res_dict.get('scraper_url') else '-'},
...
def get_package_fields(package, pkg_extras, dataset_type):
    ...
    if secondary_themes:
        try:
            ...
            secondary_themes = ', '.join(
                [THEMES.get(theme-secondary, theme-secondary) \
    ...
    field_value_map = {
        # field_name : {display info}
        'state': {'label': 'Stanje', 'value': c.pkg.state},
        'harvest-url': {'label': 'Pokupi URL', 'value': harvest_url},
        'harvest-date': {'label': 'Pokupi Datum', 'value': harvest_date},
        'harvest-guid': {'label': 'Pokupi GUID', 'value': harvest_guid},
        'bbox': {'label': 'Opseg', 'value': t.literal('Latitude: %s&deg; to %s&deg; <br/> Longitude: %s&deg; to %s&deg;' % (pkg_extras.get('bbox-north-lat'), pkg_extras.get('bbox-south-lat'), pkg_extras.get('bbox-west-long'), pkg_extras.get('bbox-east-long'))) },
        'categories': {'label': 'ONS kategorija', 'value': pkg_extras.get('categories')},
        'date_updated': {'label': 'Datum zadnje izmjene', 'value': DateType.db_to_form(pkg_extras.get('date_updated', ''))},
        'date_released': {'label': 'Datum zadnje objave', 'value': DateType.db_to_form(pkg_extras.get('date_released', ''))},
        'temporal_coverage': {'label': 'Vremensko razdoblje', 'value': temporal_coverage},
        'geographic_coverage': {'label': 'Geografska pokrivenost', 'value': GeoCoverageType.strip_off_binary(pkg_extras.get('geographic_coverage', ''))},
        'resource-type': {'label': 'Gemini2 tip resursa', 'value': pkg_extras.get('resource-type')},
        'spatial-data-service-type': {'label': 'Gemini2 tip usluge', 'value': pkg_extras.get('spatial-data-service-type')},
        'access_constraints': {'label': 'Pristupne restrikcije', 'value': render_json(pkg_extras.get('access_constraints'))},
        'taxonomy_url': {'label': 'URL Taksonomija', 'value': taxonomy_url},
        'theme': {'label': 'Tema', 'value': primary_theme},
        'theme-secondary': {'label': 'Sekundarna tema(e)', 'value': secondary_themes},
        'metadata-language': {'label': 'Jezik metapodataka', 'value': pkg_extras.get('metadata-language', '').replace('hrv', 'Hrvatski')},
        'metadata-date': {'label': 'Datum metapodataka', 'value': DateType.db_to_form(pkg_extras.get('metadata-date', ''))},
        'dataset-reference-date': {'label': 'Referentni datum skupa podataka', 'value': dataset_reference_date},
        '': {'label': '', 'value': ''},
    }
...
def license_choices(data):
    license_ids = ['odc-odbl', 'cc-zero', 'cc-by', 'cc-by-sa']
...
def search_facets_unselected(facet_keys,sort_by='count'):
    ...
    if 'publisher' in facet_keys and request.params.get('publisher',''):
        ...
        unselected.append( (link,u'Uključi podizdavače',None) )
    return unselected
...
def search_facets_selected(facet_keys):
    ...
    if 'parent_publishers' in facet_keys and request.params.get('parent_publishers',''):
        ...
        selected.append( (link,u'Uključi podizdavače',None) )
    return selected
...
def search_facet_text(key,value):
    ...
    if key=='unpublished':
        if value=='true':
            return 'Neobjavljeni skupovi podataka'
        return 'Objavljeni skupovi podataka'
    if key=='license_id-is-ogl':
        if value=='true':
            return 'OGL'
        elif value=='unpublished':
            return 'Neobjavljeni skup podataka'
