...
var costco = function() {
  ...
  function previewTransform(docs, editFunc, currentColumn) {
    ...
    for (var i = 0; i < updated.docs.length; i++) {
      var before = docs[i], after = updated.docs[i];
      if (!after) after = {};
      if (currentColumn) {
...
//
// @property {string} resource_id: The ID of the resource being displayed
//
...
my.DataExplorer = Backbone.View.extend({
  ...
  initialize: function(options) {
    ...
    this.model.bind('query:start', function() {
        my.notify('Učitavanje podataka...', {loader: true});
      });
    this.model.bind('query:done', function() {
        my.clearNotifications();
        self.el.find('.doc-count').text(self.model.docCount || 'Unknown');
        my.notify('Podaci su uspješno učitani...', {category: 'success'});
        // update navigation
...
(function($, my) {
  ...
  my.Base = Backbone.Model.extend({
    ...
    _makeRequest: function(data) {
      ...
      var extended_data = _.extend(extras, data);
      return $.ajax(extended_data);
    },
    ...
    _wrapInTimeout: function(ourFunction) {
      ...
      var timer = setTimeout(function() {
        dfd.reject({
          message: 'Poslužitelj nije odgovorio ' + (timeout / 1000) + ' sekundi'
        });
      }, timeout);
...
(function($, my) {
  ...
  my.DataProxy = my.Base.extend({
    ...
    query: function(dataset, queryObj) {
      var self = this;
      var base = this.get('dataproxy_url') + dataset.resource_id;
      var data = {
