...
$(function() {
  ...
  $('.star-rating').each(function(i,el) {
    el = $(el);
    el.tooltip({
      title: el.find('.tooltip').html() ; //el.find('.tooltip').html().replace('Content of file appeared to be format', 'Podaci su u formatu').replace('which receives openness score', 'sa ocjenom otvorenosti')),
      html: true,
      placement: 'right',
